...
resource "azapi_resource" "resourceGroup" {
  ...
  name      = "acctestRG-221102105223156420"
  location  = "westeurope"
  body      = jsonencode({})
  tags      = {}
}
...
resource "azapi_resource" "proximityPlacementGroup" {
  ...
  name      = "accPPG-221102105223156420"
...
resource "azapi_resource" "virtualNetwork" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctvn-221102105223156420"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
      addressSpace = {
...
resource "azapi_resource" "subnet" {
  ...
  name      = "acctsub-221102105223156420"
...
resource "azapi_resource" "networkInterface" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctni-221102105223156420"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
