...
resource "azapi_resource" "resourceGroup" {
  ...
  name      = "acctestRG-dtl-221102104702271469"
  location  = "westeurope"
  body      = jsonencode({})
  tags      = {}
}
...
resource "azapi_resource" "virtualNetwork" {
  ...
  name      = "acctestVN-221102104702271469"
...
resource "azapi_resource" "subnet" {
  ...
  name      = "acctestSN-221102104702271469"
...
resource "azapi_resource" "networkInterface" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctestNIC-221102104702271469"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
...
resource "azapi_resource" "virtualMachine" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctestVM-221102104702271469"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
      ...
      osProfile = {
        ...
        computerName             = "acctestVM-221102104702271469"
        linuxConfiguration = {
      ...
      storageProfile = {
        ...
        osDisk = {
          ...
          name                    = "myosdisk-221102104702271469"
          osType                  = "Linux"
...
resource "azapi_resource" "schedule" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "shutdown-computevm-acctestVM-221102104702271469"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
      dailyRecurrence = {
