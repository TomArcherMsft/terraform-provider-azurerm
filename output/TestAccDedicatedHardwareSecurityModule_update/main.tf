...
resource "azapi_resource" "resourceGroup" {
  ...
  name      = "acctestRG-hsm-221102105023570150"
  location  = "westeurope"
  body      = jsonencode({})
  tags      = {}
}
...
resource "azapi_resource" "publicIPAddress" {
  ...
  name      = "acctest-pip-221102105023570150"
...
resource "azapi_resource" "subnet2" {
  type      = "Microsoft.Network/virtualNetworks/subnets@2021-08-01"
  parent_id = azapi_resource.virtualNetwork.id
  name      = "acctest-hsmsubnet-221102105023570150"
...
resource "azapi_resource" "virtualNetworkGateway" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctest-vnetgateway-221102105023570150"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
      activeActive           = false
