...
resource "azapi_resource" "resourceGroup" {
  ...
  name      = "acctestRG-cdn-afdx-221102104512986873"
  location  = "westeurope"
  body      = jsonencode({})
  tags      = {}
}
...
resource "azapi_resource" "profile" {
  ...
  name      = "acctest-profile-221102104512986873"
  location  = "global"
  body = jsonencode({
    properties = {
...
resource "azapi_resource" "originGroup" {
  ...
  name      = "accTest-origin-group-221102104512986873"
...
resource "azapi_resource" "origin" {
  ...
  name      = "acctest-origin-221102104512986873"
