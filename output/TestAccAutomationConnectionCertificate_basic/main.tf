...
resource "azapi_resource" "resourceGroup" {
  ...
  name      = "acctestRG-auto-221102104154983414"
  location  = "westeurope"
  body      = jsonencode({})
  tags      = {}
}
...
resource "azapi_resource" "automationAccount" {
  ...
  name      = "acctestAA-221102104154983414"
...
resource "azapi_resource" "certificate" {
  ...
  name      = "acctest-221102104154983414"
...
resource "azapi_resource" "connection" {
  ...
  parent_id = azapi_resource.automationAccount.id
  name      = "acctestACC-221102104154983414"
  ...
  body = jsonencode({
    properties = {
      ...
      fieldDefinitionValues = {
        AutomationCertificateName = azapi_resource.certificate.name
