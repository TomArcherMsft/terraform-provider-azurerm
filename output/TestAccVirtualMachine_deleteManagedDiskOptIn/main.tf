...
resource "azapi_resource" "resourceGroup" {
  ...
  name      = "acctestRG-221102105127959831"
  location  = "westeurope"
  body      = jsonencode({})
  tags      = {}
}
...
resource "azapi_resource" "virtualNetwork" {
  ...
  name      = "acctvn-221102105127959831"
...
resource "azapi_resource" "subnet" {
  ...
  name      = "acctsub-221102105127959831"
...
resource "azapi_resource" "networkInterface" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctni-221102105127959831"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
...
resource "azapi_resource" "virtualMachine" {
  ...
  parent_id = azapi_resource.resourceGroup.id
  name      = "acctvm-221102105127959831"
  location  = azapi_resource.resourceGroup.location
  body = jsonencode({
    properties = {
      hardwareProfile = {
      ...
      osProfile = {
        ...
        computerName  = "hn221102105127959831"
        linuxConfiguration = {
